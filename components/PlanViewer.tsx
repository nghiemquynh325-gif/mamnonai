        ...
        <div className="flex flex-wrap gap-2 justify-center">
          <button
            onClick={async () => {
              if (!request) return;
              try {
                const { savePlan } = await import('../services/storageService');
                await savePlan(request, content);
                alert("Đã lưu giáo án thành công! Cô có thể xem lại ở Dashboard.");
              } catch (err: any) {
                console.error("Manual save error:", err);
                alert(err.message || "Lỗi khi lưu. Cô vui lòng kiểm tra mạng hoặc đăng nhập lại.");
              }
            }}
            className="flex items-center space-x-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors text-sm font-medium shadow-md"
          >
            <CheckCheck size={16} />
